...
export class Player extends BaseGameObject<ObjectCategory.Player> {
    ...
    constructor(game: Game, socket: WebSocket<PlayerContainer>, position: Vector, team?: Team) {
        ...
        if (specialFunnies) {
            ...
            this.game.addLoot("4x_scope", this.position);
            this.game.addLoot("8x_scope", this.position);
            this.game.addLoot("15x_scope", this.position);
    ...
    update(): void {
        ...
        // Regenerate health
        if (this._adrenaline >= 250) this.health += dt * 25 / (30 ** 2);
        else if (this._adrenaline >= 200) this.health += dt * 18 / (30 ** 2);
        else if (this._adrenaline >= 150) this.health += dt * 12 / (30 ** 2);
        else if (this._adrenaline >= 100) this.health += dt * 5 / (30 ** 2);
        else if (this._adrenaline >= 87.5) this.health += dt * 2.75 / (30 ** 2);
        ...
        if (this.startedAttacking) {
            ...
            // in case of throwables used, refill the player's throwable inventory
            // this somehow sets it to unlimited tho idk
            if(this.activeItem.category === 3) {
                this.inventory.items.setItem('frag_grenade', this.inventory.backpack.maxCapacity['frag_grenade']);
                this.inventory.items.setItem('smoke_grenade', this.inventory.backpack.maxCapacity['smoke_grenade']);
            }
